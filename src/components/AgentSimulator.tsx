...
export default function AgentSimulator({ initialAgentId }: AgentSimulatorProps) {
  ...
  /**
   * Action principale : Envoi du message en priorité au proxy sécurisé /api/chat (Vercel)
   * avec rabattement direct sur l'API client si nécessaire.
   */
  const handleSendMessage = async (e: FormEvent) => {
    ...
    // Définir les instructions systèmes de l'agent actif
    ...
    try {
      let replyText = "";
      addLog(`API_POST >> Requête envoyée au proxy Vercel (/api/chat)...`);

      try {
        // Tentative d'appel au serveur Vercel Serverless
        ...
        if (proxyResponse.ok) {
          ...
          addLog(`GEMINI_RESPONSE >> Succès via proxy backend (/api/chat).`);
        } else {
          throw new Error(`HTTP_${proxyResponse.status}`);
        }
      } catch (proxyErr: any) {
        // En cas d'échec du proxy (ex: développement local), rabattement sur le SDK direct
        addLog(`LOCAL_SDK_FALLBACK >> /api/chat indisponible (${proxyErr.message || proxyErr}). Appel direct Gemini SDK...`);

        const apiKey = ((import.meta as any).env?.VITE_GEMINI_API_KEY) || ((import.meta as any).env?.GEMINI_API_KEY) || "";
        if (!apiKey) {
          throw new Error("Clé API Gemini absente de l'environnement.");
        }
        ...
        replyText = response.text || "Désolé, je n'ai pas pu formuler de réponse.";
        addLog(`GEMINI_RESPONSE >> Succès en direct (SDK client).`);
      }
      ...
      const errorMsg: ChatMessage = {
        id: `sys-err-${Date.now()}`,
        sender: 'system',
        content: `❌ Impossible de connecter l'Agent à Gemini.\n\nDétail de l'erreur : ${err.message || err}\n\nSolutions :\n1. Vérifiez que vous avez bien configuré la variable "GEMINI_API_KEY" dans vos paramètres de déploiement (Vercel ou autre).\n2. En développement local, assurez-vous d'avoir créé le fichier .env.local contenant votre clé.`,
        timestamp: new Date().toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' }),
      };
